...
@section('pageTitle', 'Create New Reminder')
  ...
  <div class="col-12">
    <div class="card">
      <div class="card-header">
        <h4 class="card-title">Create New Reminder</h4>
      ...
      <div class="card-body">
        @if (Route::has('admin.reminders.store'))
<form action="{{ route('admin.reminders.store') }}" method="POST" enctype="multipart/form-data">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control @error('completed') is-invalid @enderror" 
                   id="completed" name="completed" value="{{ old('completed') }}">
            @error('completed')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
