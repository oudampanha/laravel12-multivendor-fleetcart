...
@section('pageTitle', 'Create New Cart')
  ...
  <div class="col-12">
    <div class="card">
      <div class="card-header">
        <h4 class="card-title">Create New Cart</h4>
      ...
      <div class="card-body">
        @if (Route::has('admin.carts.store'))
<form action="{{ route('admin.carts.store') }}" method="POST" enctype="multipart/form-data">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control @error('items_count') is-invalid @enderror" 
                   id="items_count" name="items_count" value="{{ old('items_count') }}">
            @error('items_count')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
