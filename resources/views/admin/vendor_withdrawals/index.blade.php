        ...
        <div class="card-body">
          <div class="table-responsive">
            <table class="table table-striped table-bordered" id="vendorWithdrawalsTable">
              ...
              <tbody>
                @foreach ($vendorWithdrawals as $withdrawal)
                  <tr>
                    <td>{{ $withdrawal->id }}</td>
                    <td>
                      <a href="{{ route('admin.vendors.show', $withdrawal->vendor_id) }}" class="text-primary">
                        {{ $withdrawal->vendor->user->first_name ?? 'N/A' }}
                    ...
                    <td>
                      <div class="btn-group">
                        <a href="{{ route('admin.vendor-withdrawals.show', $withdrawal->id) }}"
                          class="btn btn-sm btn-info">
                          <i class="fas fa-eye"></i>
                        </a>
                        @if ($withdrawal->status === 'pending')
                          <a href="{{ route('admin.vendor-withdrawals.edit', $withdrawal->id) }}"
  ...
  <script>
    $(document).ready(function() {
      $('#vendorWithdrawalsTable').DataTable({
        ...
        pageLength: 25,
        language: {
          emptyTable: 'No vendor withdrawals found'
        }
      });
    });
  </script>
