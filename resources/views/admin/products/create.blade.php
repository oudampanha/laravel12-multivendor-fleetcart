        ...
        <div class="row">
          ...
          <div class="col-md-4">
            ...
            <div class="card mb-4">
              ...
              <input type="file" id="mediaUpload" name="media[]" multiple accept="image/*" style="display: none;">
            </div>
          </div>
...
@push('scripts')
  ...
  <script>
    ...
    `;
      ...
      // Handle clicking on main image to upload
      $('.main-image-holder, .add-new-media').on('click', function() {
        $('#mediaUpload').click();
      });
      ...
      $('#mediaUpload').on('change', function(e) {
        ...
        if (files.length > 0) {
          ...
          // you'd upload the files via AJAX and update the display
        }
      });

      // Handle remove image buttons
      $(document).on('click', '.remove-main-image, .remove-thumbnail', function(e) {
        e.stopPropagation();
        const $item = $(this).closest('.main-image-holder, .media-thumbnail-item');

        // Show confirmation dialog
        if (confirm('Are you sure you want to remove this image?')) {
          if ($(this).hasClass('remove-main-image')) {
            // Remove main image - you might want to set a default or placeholder
            $(this).siblings('.main-product-image').attr('src',
              'https://asia.fleetcart.envaysoft.com/build/assets/placeholder_image-BqsW5LDU-v4.7.6.png');
          } else {
            // Remove thumbnail
            $item.remove();
          }
      ...
      $(document).on('click', '.media-thumbnail-item:not(.add-new-media)', function(e) {
        if (!$(e.target).hasClass('remove-thumbnail') && !$(e.target).closest('.remove-thumbnail').length) {
          ...
          const currentMainSrc = $('.main-product-image').attr('src');

          // Swap images
          $('.main-product-image').attr('src', thumbnailSrc);
      ...
      if (typeof $.fn.sortable !== 'undefined') {
        $('.media-thumbnails-grid').sortable({
