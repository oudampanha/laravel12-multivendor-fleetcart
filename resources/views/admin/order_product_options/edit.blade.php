...
@section('pageTitle', 'Edit Order Product Option')
        ...
        <form action="{{ route('admin.order-product-options.update', $item->id ?? 0) }}" method="POST" enctype="multipart/form-data">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control @error('option') is-invalid @enderror" 
                   id="option" name="option" value="{{ old('option', $item->option ?? '') }}">
            @error('option')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control @error('option_value') is-invalid @enderror" 
                   id="option_value" name="option_value" value="{{ old('option_value', $item->option_value ?? '') }}">
            @error('option_value')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
